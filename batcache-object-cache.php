 ...
 */
function bc_cache_add( $key, $value, $group = '', $expiration = 0 ) {
	global $bc_object_cache;
	return $bc_object_cache->add( $key, $value, $group, $expiration );
}
 ...
 * @return bool                     Returns TRUE on success or FALSE on failure.
 */
function bc_cache_add_by_key( $server_key, $key, $value, $group = '', $expiration = 0 ) {
	global $bc_object_cache;
	return $bc_object_cache->addByKey( $server_key, $key, $value, $group, $expiration );
}
 ...
 * @return bool                 Returns TRUE on success or FALSE on failure.
 */
function bc_cache_add_server( $host, $port, $weight = 0 ) {
	global $bc_object_cache;
	return $bc_object_cache->addServer( $host, $port, $weight );
}
 ...
 */
function bc_cache_add_servers( $servers ) {
	global $bc_object_cache;
	return $bc_object_cache->addServers( $servers );
}
 ...
 * @return bool             Returns TRUE on success or FALSE on failure.
 */
function bc_cache_append( $key, $value, $group = '' ) {
	global $bc_object_cache;
	return $bc_object_cache->append( $key, $value, $group );
}
 ...
 * @return bool                     Returns TRUE on success or FALSE on failure.
 */
function bc_cache_append_by_key( $server_key, $key, $value, $group = '' ) {
	global $bc_object_cache;
	return $bc_object_cache->appendByKey( $server_key, $key, $value, $group );
}
 ...
 * @return bool                 Returns TRUE on success or FALSE on failure.
 */
function bc_cache_cas( $cas_token, $key, $value, $group = '', $expiration = 0 ) {
	global $bc_object_cache;
	return $bc_object_cache->cas( $cas_token, $key, $value, $group, $expiration );
}
 ...
 * @return bool                 Returns TRUE on success or FALSE on failure.
 */
function bc_cache_cas_by_key( $cas_token, $server_key, $key, $value, $group = '', $expiration = 0 ) {
	global $bc_object_cache;
	return $bc_object_cache->casByKey( $cas_token, $server_key, $key, $value, $group, $expiration );
}
 ...
 */
function bc_cache_decrement( $key, $offset = 1, $group = '' ) {
	global $bc_object_cache;
	return $bc_object_cache->decrement( $key, $offset, $group );
}

/**
 * Decrement a numeric item's value.
 *
 * Same as bc_cache_decrement. Original WordPress caching backends use bc_cache_decr. I
 ...
 * @return int|bool         Returns item's new value on success or FALSE on failure.
 */
function bc_cache_decr( $key, $offset = 1, $group = '' ) {
	return bc_cache_decrement( $key, $offset, $group );
}
 ...
 * @return bool             Returns TRUE on success or FALSE on failure.
 */
function bc_cache_delete( $key, $group = '', $time = 0 ) {
	global $bc_object_cache;
	return $bc_object_cache->delete( $key, $group, $time );
}

/**
 * Removes items from the cache.
 *
 * Remove a list of items from memcached. The
 * $time parameter allows an object to be queued for deletion without immediately
 * deleting. Between the time that it is queued and the time it's deleted, add,
 * replace, and get will fail, but set will succeed.
 *
 * @link http://www.php.net/manual/en/memcached.delete_multi.php
 *
 * @param string    $key    The key under which to store the value.
 * @param string    $group  The group value appended to the $key.
 * @param int       $time   The amount of time the server will wait to delete the item in seconds.
 * @return bool             Returns TRUE on success or FALSE on failure.
 */
function bc_cache_delete_multi( $key, $group = '', $time = 0 ) {
	global $bc_object_cache;
	return $bc_object_cache->delete_multi( $key, $group, $time );
}
 ...
 * @return bool                     Returns TRUE on success or FALSE on failure.
 */
function bc_cache_delete_by_key( $server_key, $key, $group = '', $time = 0 ) {
	global $bc_object_cache;
	return $bc_object_cache->deleteByKey( $server_key, $key, $group, $time );
}
 ...
 */
function bc_cache_fetch() {
	global $bc_object_cache;
	return $bc_object_cache->fetch();
}
 ...
 */
function bc_cache_fetch_all() {
	global $bc_object_cache;
	return $bc_object_cache->fetchAll();
}
 ...
 * @return bool             Returns TRUE on success or FALSE on failure.
 */
function bc_cache_flush( $delay = 0 ) {
	global $bc_object_cache;
	return $bc_object_cache->flush( $delay );
}
 ...
 *
 * Note that the $deprecated and $found args are only here for compatibility with the native bc_cache_get function.
 *
 ...
 */
function bc_cache_get( $key, $group = '', $force = false, &$found = null, $cache_cb = null, &$cas_token = null ) {
	global $bc_object_cache;

	if ( func_num_args() > 4 )
		return $bc_object_cache->get( $key, $group, $force, $found, '', false, $cache_cb, $cas_token );
	else
		return $bc_object_cache->get( $key, $group, $force, $found );
}
 ...
 * @return bool|mixed               Cached object value.
 */
function bc_cache_get_by_key( $server_key, $key, $group = '', $force = false, &$found = null, $cache_cb = NULL, &$cas_token = NULL ) {
	global $bc_object_cache;

	if ( func_num_args() > 5 )
		return $bc_object_cache->getByKey( $server_key, $key, $group, $force, $found, $cache_cb, $cas_token );
	else
		return $bc_object_cache->getByKey( $server_key, $key, $group, $force, $found );
}
 ...
 * @return bool                     Returns TRUE on success or FALSE on failure.
 */
function bc_cache_get_delayed( $keys, $groups = '', $with_cas = false, $value_cb = NULL ) {
	global $bc_object_cache;
	return $bc_object_cache->getDelayed( $keys, $groups, $with_cas, $value_cb );
}
 ...
 * @return bool                     Returns TRUE on success or FALSE on failure.
 */
function bc_cache_get_delayed_by_key( $server_key, $keys, $groups = '', $with_cas = false, $value_cb = NULL ) {
	global $bc_object_cache;
	return $bc_object_cache->getDelayedByKey( $server_key, $keys, $groups, $with_cas, $value_cb );
}
 ...
 */
function bc_cache_get_multi( $keys, $groups = '', &$cas_tokens = NULL, $flags = NULL ) {
	global $bc_object_cache;

	if ( func_num_args() > 2 )
		return $bc_object_cache->getMulti( $keys, $groups, '', $cas_tokens, $flags );
	else
		return $bc_object_cache->getMulti( $keys, $groups );
}
 ...
 * @return bool|array               Returns the array of found items or FALSE on failure.
 */
function bc_cache_get_multi_by_key( $server_key, $keys, $groups = '', &$cas_tokens = NULL, $flags = NULL ) {
	global $bc_object_cache;

	if ( func_num_args() > 3 )
		return $bc_object_cache->getMultiByKey( $server_key, $keys, $groups, $cas_tokens, $flags );
	else
		return $bc_object_cache->getMultiByKey( $server_key, $keys, $groups );
}
 ...
 */
function bc_cache_get_option( $option ) {
	global $bc_object_cache;
	return $bc_object_cache->getOption( $option );
}
 ...
 */
function bc_cache_get_result_code() {
	global $bc_object_cache;
	return $bc_object_cache->getResultCode();
}
 ...
 */
function bc_cache_get_result_message() {
	global $bc_object_cache;
	return $bc_object_cache->getResultMessage();
}
 ...
 */
function bc_cache_get_server_by_key( $server_key ) {
	global $bc_object_cache;
	return $bc_object_cache->getServerByKey( $server_key );
}
 ...
 */
function bc_cache_get_server_list() {
	global $bc_object_cache;
	return $bc_object_cache->getServerList();
}
 ...
 */
function bc_cache_get_stats() {
	global $bc_object_cache;
	return $bc_object_cache->getStats();
}
 ...
 */
function bc_cache_get_version() {
	global $bc_object_cache;
	return $bc_object_cache->getVersion();
}
 ...
 * @return int|bool         Returns item's new value on success or FALSE on failure.
 */
function bc_cache_increment( $key, $offset = 1, $group = '' ) {
	global $bc_object_cache;
	return $bc_object_cache->increment( $key, $offset, $group );
}

/**
 * Increment a numeric item's value.
 *
 * This is the same as bc_cache_increment, but kept for back compatibility. The original
 * WordPress caching backends use bc_cache_incr. I want both to work.
 *
 * @link http://www.php.net/manual/en/memcached.increment.php
 *
 * @param string    $key    The key under which to store the value.
 ...
 * @return int|bool         Returns item's new value on success or FALSE on failure.
 */
function bc_cache_incr( $key, $offset = 1, $group = '' ) {
	return bc_cache_increment( $key, $offset, $group );
}
 ...
 * @return bool             Returns TRUE on success or FALSE on failure.
 */
function bc_cache_prepend( $key, $value, $group = '' ) {
	global $bc_object_cache;
	return $bc_object_cache->prepend( $key, $value, $group );
}
 ...
 * @return bool                     Returns TRUE on success or FALSE on failure.
 */
function bc_cache_prepend_by_key( $server_key, $key, $value, $group = '' ) {
	global $bc_object_cache;
	return $bc_object_cache->prependByKey( $server_key, $key, $value, $group );
}
 ...
 * @return bool                 Returns TRUE on success or FALSE on failure.
 */
function bc_cache_replace( $key, $value, $group = '', $expiration = 0 ) {
	global $bc_object_cache;
	return $bc_object_cache->replace( $key, $value, $group, $expiration );
}
 ...
 * @return bool                     Returns TRUE on success or FALSE on failure.
 */
function bc_cache_replace_by_key( $server_key, $key, $value, $group = '', $expiration = 0 ) {
	global $bc_object_cache;
	return $bc_object_cache->replaceByKey( $server_key, $key, $value, $group, $expiration );
}
 ...
 * @return bool                 Returns TRUE on success or FALSE on failure.
 */
function bc_cache_set( $key, $value, $group = '', $expiration = 0 ) {
	global $bc_object_cache;
	return $bc_object_cache->set( $key, $value, $group, $expiration );
}
 ...
 * @return bool                     Returns TRUE on success or FALSE on failure.
 */
function bc_cache_set_by_key( $server_key, $key, $value, $group = '', $expiration = 0 ) {
	global $bc_object_cache;
	return $bc_object_cache->setByKey( $server_key, $key, $value, $group, $expiration );
}
 ...
 * @return bool                     Returns TRUE on success or FALSE on failure.
 */
function bc_cache_set_multi( $items, $groups = '', $expiration = 0 ) {
	global $bc_object_cache;
	return $bc_object_cache->setMulti( $items, $groups, $expiration );
}
 ...
 * @return bool                     Returns TRUE on success or FALSE on failure.
 */
function bc_cache_set_multi_by_key( $server_key, $items, $groups = 'default', $expiration = 0 ) {
	global $bc_object_cache;
	return $bc_object_cache->setMultiByKey( $server_key, $items, $groups, $expiration );
}
 ...
 * @return bool             Returns TRUE on success or FALSE on failure.
 */
function bc_cache_set_option( $option, $value ) {
	global $bc_object_cache;
	return $bc_object_cache->setOption( $option, $value );
}
 ...
 * @return void
 */
function bc_cache_switch_to_blog( $blog_id ) {
	global $bc_object_cache;
	return $bc_object_cache->switch_to_blog( $blog_id );
}
 ...
 *
 * @global  bc_object_cache     $bc_object_cache    WordPress Object Cache
 * @return  void
 */
function bc_cache_init() {
	global $bc_object_cache;
	$bc_object_cache = new bc_object_cache();
}
 ...
 * @return  void
 */
function bc_cache_add_global_groups( $groups ) {
	global $bc_object_cache;
	$bc_object_cache->add_global_groups( $groups );
}
 ...
 * @return  void
 */
function bc_cache_add_non_persistent_groups( $groups ) {
	global $bc_object_cache;
	$bc_object_cache->add_non_persistent_groups( $groups );
}

class bc_object_cache {
	...
	/**
	 * Flag to temporarily stop processing calls to delete()
	 * @var bool
	 */
	public $ignore_deletes = false;
	...
	public function __construct( $persistent_id = NULL ) {
		 ...
		 */
		if ( ! defined( 'BC_CACHE_KEY_SALT' ) )
			define( 'BC_CACHE_KEY_SALT', '' );
	 ...
	 * @link http://www.php.net/manual/en/memcached.delete_multi.php
	 *
	 * @param array     $keys   The key under which to store the value.
	 * @param string    $group  The group value appended to the $key.
	 * @param int       $time   The amount of time the server will wait to delete the item in seconds.
	 * @return bool             Returns TRUE on success or FALSE on failure.
	 */
	public function delete_multi( $keys, $group = 'default', $time = 0 ) {

		$final_keys = array();

		foreach ( $keys as $key ) {
			$derived_key  = $this->buildKey( $key, $group );
			$final_keys[] = $derived_key;
			unset( $this->cache[ $derived_key ] );
		}

		$result = $this->m->deleteMulti( $final_keys, $time );
	 ...
	 *
	 * Certain plugins expect an "incr" method on the $bc_object_cache object (e.g., Batcache). Since the original
	...
	public function buildKey( $key, $group = 'default' ) {
		...
		return preg_replace( '/\s+/', '', BC_CACHE_KEY_SALT . "$prefix$group:$key" );
	}
